...
export class ClientUploadFileServices {
    ...
    public static async uploadFile(file: File): Promise<UploadFileDto> {
        ...
        const validationFile = SecureFileUploadServices.validationFile(resultFile);
    ...
    public static async uploadFiles(files: File[]): Promise<UploadFilesDto> {
        try {
            ...
            const resultFiles = await Promise.all(
                validFiles.map(async file => await this.uploadFileFunc(file))
            );

            // validation file
            const errorMessage = await SecureFileUploadServices.validationFiles(resultFiles);
